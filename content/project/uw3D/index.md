...
Any NeRF-based architecture requires RBG information and camera poses. As we intend to utilize multi-modal data, we require sonar data along with sonar poses. [COLMAP](https://colmap.github.io/) is a general-purpose Structure-from-Motion and Multi-View Stereo pipeline with a graphical and command-line interface. It is intended for RGB input and cannot take in sonar data to output sonar poses. So, I present an optimization problem that requires sonar data [D_s] consisting of range and intensities (r, θ), 3D points x = (Xc, Yc, Zc) sampled by the camera and camera poses Pcam as input and produces the transformation matrix from camera to sonar, Tson cam along with a scaling factor, α. Pre-multiplying this transformation matrix with the camera poses produces sonar poses assuming that the sonar sampled the same 3D points as the camera.
